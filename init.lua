...
local function createHiddenHook(originalFunction, hookFunction)
    local hiddenHook = newcclosure(function(...)
        return hookFunction(originalFunction, ...)
    end)
    return hiddenHook
end

local function createDynamicHook(originalFunction, hookFunction)
    local dynamicHook = function(...)
        local hookResult, result = Pcall(hookFunction, originalFunction, ...)
        if hookResult then
            return result(...)
        else
...
__namecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    ...
    __request = createDynamicHook(reqfunc, function(originalRequest, req)
        if Type(req) ~= "table" then return originalRequest(req) end
        ...
        if Type(RequestData.Url) ~= "string" then return originalRequest(req) end
        ...
        cwrap(function()
            ...
            local ok, ResponseData = Pcall(originalRequest, RequestData)
            if not ok then
